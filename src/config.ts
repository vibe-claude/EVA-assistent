...
const DEFAULT_SETTINGS: Settings = {
  ...
  session: {
    ...
    maxMessages: 30,
    maxAgeHours: 24,
    summaryPath: join(process.cwd(), "memory", "summaries"),
  },
};
...
function parseSettings(raw: Record<string, any>): Settings {
  ...
  return {
    ...
    session: {
      ...
      maxMessages: Number.isFinite(raw.session?.maxMessages) ? Number(raw.session.maxMessages) : 30,
      maxAgeHours: Number.isFinite(raw.session?.maxAgeHours) ? Number(raw.session.maxAgeHours) : 24,
      summaryPath: typeof raw.session?.summaryPath === "string" ? raw.session.summaryPath : join(process.cwd(), "memory", "summaries"),
    },
  };
}
