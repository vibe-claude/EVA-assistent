...
import { getSession, createSession, incrementTurn, markCompactWarned, peekSession } from "./sessions";
...
async function execClaude(name: string, prompt: string, threadId?: string): Promise<RunResult> {
  ...
  // Check if session needs rotation before processing
  if (!threadId) {
    const currentSession = await peekSession();
    if (currentSession) {
      const settings = getSettings();
      if (needsRotation(currentSession, settings.session)) {
        await rotateSession(settings.session);
      }
    }
  }
