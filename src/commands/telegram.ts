...
// --- Message batching (collect docs/images from same user for 3s before processing) ---
...
interface BatchState {
  textMessage: TelegramMessage | null;
  entries: BatchEntry[];
  timer: ReturnType<typeof setTimeout>;
}

const messageBatches = new Map<string, BatchState>();

function batchKey(chatId: number, threadId?: number): string {
  return threadId ? `${chatId}:${threadId}` : String(chatId);
}

async function prepareEntry(token: string, message: TelegramMessage): Promise<BatchEntry | null> {
  ...
  if (!hasImage && !hasDocument) return null;

  let imagePath: string | null = null;
  let documentInfo: { localPath: string; originalName: string } | null = null;

  if (hasImage) {
    try { imagePath = await downloadImageFromMessage(token, message); } catch {}
  }
  if (hasDocument) {
    try { documentInfo = await downloadDocumentFromMessage(token, message); } catch {}
  }
  return { message, imagePath, documentInfo };
}

async function flushBatch(key: string): Promise<void> {
  const state = messageBatches.get(key);
  if (!state) return;
  messageBatches.delete(key);

  const { textMessage, entries } = state;
  const config = getSettings().telegram;

  // Use the first message as source of chatId/threadId/label
  const anchor = textMessage ?? entries[0]?.message;
  if (!anchor) return;

  const chatId = anchor.chat.id;
  const threadId = anchor.message_thread_id;
  const label = anchor.from?.username ?? String(anchor.from?.id ?? "unknown");
  const { text } = getMessageTextAndEntities(textMessage ?? anchor);
  ...
  try {
    ...
    // Append all documents/images in batch
    for (const entry of entries) {
      if (entry.imagePath) {
        promptParts.push(`Image path: ${entry.imagePath}`);
        promptParts.push(`Original filename: ${entry.message.document?.file_name ?? "image"}`);
        promptParts.push(`The user attached an image. DO NOT read this file directly into the session — it inflates context with base64. Use subprocess via claude -p --no-session-persistence to extract content, then ask the user: 'Сохранить оригинал файла?' Follow ${join(process.cwd(), "skills", "docs.md")}.`);
      }
      if (entry.documentInfo) {
        promptParts.push(`Document path: ${entry.documentInfo.localPath}`);
        promptParts.push(`Original filename: ${entry.documentInfo.originalName}`);
        promptParts.push(`The user attached a document. DO NOT read this file directly into the session — use subprocess via claude -p --no-session-persistence to extract content, then ask the user: 'Сохранить оригинал файла?' Follow ${join(process.cwd(), "skills", "docs.md")}.`);
      }
    }

    if (entries.length > 1) {
      promptParts.push(`Note: the user sent ${entries.length} files in one batch. Process all of them, then send ONE final summary message.`);
    }
    ...
    if (result.exitCode !== 0) {
      ...
      if (reactionEmoji) {
        await sendReaction(config.token, chatId, anchor.message_id, reactionEmoji).catch(() => {});
      }
      if (cleanedText) await sendMessage(config.token, chatId, cleanedText, threadId);
    ...
    console.error(`[Telegram] Batch error for ${label}: ${errMsg}`);
...
async function handleMessage(message: TelegramMessage): Promise<void> {
  ...
  // --- Batch documents/images: collect for 3s then process together ---
  if ((hasImage || hasDocument) && !hasVoice && !command) {
    const key = batchKey(chatId, threadId);
    // Download file eagerly so we don't block the timer flush
    const entry = await prepareEntry(config.token, message).catch(() => null);
    if (entry) {
      const existing = messageBatches.get(key);
      if (existing) {
        clearTimeout(existing.timer);
        existing.entries.push(entry);
        if (text.trim() && !existing.textMessage) existing.textMessage = message;
        existing.timer = setTimeout(() => flushBatch(key), 3000);
      } else {
        const timer = setTimeout(() => flushBatch(key), 3000);
        messageBatches.set(key, {
          textMessage: text.trim() ? message : null,
          entries: [entry],
          timer,
        });
      }
  ...
  // If there's a pending batch for this chat and a text-only message arrives — attach it as context
  if (text.trim() && !hasImage && !hasDocument && !hasVoice) {
    const key = batchKey(chatId, threadId);
    const existing = messageBatches.get(key);
    if (existing && !existing.textMessage) {
      existing.textMessage = message;
      return;
    }
  }

  // --- Regular (non-batched) message handling ---
  const typingInterval = setInterval(() => sendTyping(config.token, chatId, threadId), 4000);
